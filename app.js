...
//lastlocattions
const lastlocationsContainer = document.getElementById('lastlocations');
...
const nowLocationData = (lat,long) =>{
    ...
    fetch(urlcurrentuserdata)
               ...
               .then(data=>{
                   ...
                   //GET : 1)location name 2:)location lan lat 
                   const localtiondata = {name:`${data.name},${data.sys.country}`,
                                         lat:data.coord.lat,
                                         lon:data.coord.lon};
                   
                   //GET : 2) Store it in local storage like this
                   localStorage.setItem(`${data.name},${data.sys.country}`,JSON.stringify(localtiondata));
...
const lastlocations = () =>{
    for (let i = 0; i < localStorage.length; i++)  
    {
        ...
        let citydata = JSON.parse(localStorage.getItem(key));//GET city object

        const div = document.createElement('div');
        div.classList = 'lastdata';
        div.innerHTML = `
        <span class="badge badge-pill badge-secondary mr-1 ml-1 font-weight-bold" id="${citydata.lat},${citydata.lon}">${citydata.name}</span>
        `;
        lastlocationsContainer.appendChild(div);
        div.addEventListener('click',(e)=>{
            const idCity =e.target.id.split(",");
            lat = Number(idCity[0]);
            long = Number(idCity[1]);
